...
public class C_EasyTouchTemplate : MonoBehaviour
{
    ...
    {
        Vector2 last = gesture.position;
        if ((start - last).magnitude > 0.1 && swipelock)
        {
            GameController._instance.Male.transform.Rotate(new Vector3(0,
                (gesture.position - gesture.startPosition).x * Time.deltaTime * scrollSpeed * -1, 0));
        }
    }
    ...
    {
        if (GameController._instance.MainCamera.fieldOfView < 60)
        {
            swipelock = false;
            GameController._instance.MainCamera.fieldOfView += gesture.deltaPinch * pinchSpeed;
        }
    }

    private void On_PinchOut(Gesture gesture)
    {
        if (GameController._instance.MainCamera.fieldOfView > 10)
        {
            swipelock = false;
            GameController._instance.MainCamera.fieldOfView -= gesture.deltaPinch * pinchSpeed;
        }
